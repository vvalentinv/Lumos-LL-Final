...
const checkDeckAuthor = (deckID, userUUID) => {
  // console.log("Check DECK AUTHOR:", deckID, userUUID);

  return client.query(`SELECT user_id FROM decks WHERE id = $1;`, [deckID])
    .then((results) => {
      // console.log("change deck visibility FROM THE DATABASE:", results)
      return results.rows[0].user_id === userUUID ? true : false;
    })
